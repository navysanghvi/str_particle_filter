...
class particle:
	...
	def get_wt_vect(self, X_upd, i, angs, inds):
		...
		for i in inds.tolist():
			...
			for j in range(len(meas_pos)):
				...
				# print 'wt_vect[j]: ' + str(wt_vect[j])
				# wt_vect[j] = wt_vect[j]*(self.zhit**2*(1-x+(x**2)/2) + self.zrand/self.lsr_max)
	...
	def get_p_upd(self, wt_vect, X_upd):
		norm_wt_vect = wt_vect/np.amax(wt_vect)
		# print np.histogram(norm_wt_vect)
